...
DATASETS = ['writing', 'english', 'german', 'pubmed', 'stack']
...
def load_stack(cache_dir=None):
    data = datasets.load_dataset('bigcode/the-stack', data_dir="data/python", streaming=True, split="train")
    samples = data.take(300)
    return [s['content'] for s in samples]
